...
(def ray-scatter-track-test
  (ray-scatter-shader-test
    (fn [program transmittance-height-size transmittance-elevation-size
        ...
        (uniform-float program :radius radius)
        (uniform-float program :max_height max-height))
    ray-scatter-track-probe ray-scatter-track shaders/ray-scatter-forward shaders/clip-angle shaders/elevation-to-index
    shaders/interpolate-4d shaders/convert-4d-index transmittance-track shaders/transmittance-forward shaders/interpolate-2d
    shaders/convert-2d-index shaders/is-above-horizon shaders/height-to-index shaders/horizon-distance shaders/limit-quot
    shaders/sun-elevation-to-index shaders/sun-angle-to-index))
