...
(facts "Integrate over a ray"
  (integral-ray #:sfsim25.ray{:origin (vec3 2 3 5) :direction (vec3 1 0 0)} 10 0 (fn [x] (vec2 2 2)))
  => (roughly-vector (vec2 0 0) 1e-6)
  (integral-ray #:sfsim25.ray{:origin (vec3 2 3 5) :direction (vec3 1 0 0)} 10 3 (fn [x] (vec2 2 2)))
  => (roughly-vector (vec2 6 6) 1e-6)
  (integral-ray #:sfsim25.ray{:origin (vec3 2 3 5) :direction (vec3 1 0 0)} 10 3 (fn [x] (vec2 (x 0) (x 0))))
