...
(mi/collect! {:ns ['sfsim25.interpolate]})
(mi/instrument! {:report (pretty/thrower)})

(tabular "1D linear mapping"
         (fact ((:sfsim25.interpolate/forward (linear-space [-2.0] [4.0] [16])) ?x) => [?result])
         ?x   ?result
         -2.0  0.0
          4.0 15.0
          0.0  5.0)
...
(fact "2D linear mapping"
      ((:sfsim25.interpolate/forward (linear-space [-2.0 -1.0] [4.0 1.0] [16 5])) 4.0 0.0) => [15.0 2.0])

(tabular "1D inverse linear sampling"
         (fact ((:sfsim25.interpolate/backward (linear-space [-2.0] [4.0] [16])) ?i) => [?result])
         ?i ?result
          0.0 -2.0
         15.0  4.0
          5.0  0.0)
...
(fact "2D inverse linear mapping"
      ((:sfsim25.interpolate/backward (linear-space [-2.0 -1.0] [4.0 1.0] [16 5])) 15 2) => [4.0 0.0])

(facts "Shape of linear space"
       (:sfsim25.interpolate/shape (linear-space [-2.0] [4.0] [16])) => [16]
       (:sfsim25.interpolate/shape (linear-space [-2.0 -1.0] [4.0 1.0] [16 5])) => [16 5])
