...
(facts "Choose NDC and texture coordinate matrices for shadow mapping"
       (let [projection      (projection-matrix 640 480 5.0 1000.0 (* 0.5 PI))
             ...
             light-direction (matrix [0 1 0])]
         (mmul (:shadow-ndc-matrix (shadow-matrices projection transform1 light-direction 0)) (matrix [0 750 -1000 1]))
         => (roughly-matrix (matrix [1 0 1 1]) 1e-6)
         (mmul (:shadow-ndc-matrix (shadow-matrices projection transform1 light-direction 0)) (matrix [0 -750 -1000 1]))
         => (roughly-matrix (matrix [1 0 0 1]) 1e-6)
         (mmul (:shadow-ndc-matrix (shadow-matrices projection transform2 light-direction 0)) (matrix [0 1000 0 1]))
         => (roughly-matrix (matrix [0 0 1 1]) 1e-6)
         (mmul (:shadow-ndc-matrix (shadow-matrices projection transform2 light-direction 0)) (matrix [0 5 0 1]))
         => (roughly-matrix (matrix [0 0 0 1]) 1e-6)
         (mmul (:shadow-map-matrix (shadow-matrices projection transform1 light-direction 0)) (matrix [0 -750 -1000 1]))
         => (roughly-matrix (matrix [1 0.5 0 1]) 1e-6)
         (mmul (:shadow-map-matrix (shadow-matrices projection transform1 light-direction 0)) (matrix [0 750 -1000 1]))
         => (roughly-matrix (matrix [1 0.5 1 1]) 1e-6)
         (mmul (:shadow-map-matrix (shadow-matrices projection transform2 light-direction 0)) (matrix [0 1000 0 1]))
         => (roughly-matrix (matrix [0.5 0.5 1 1]) 1e-6)
         (mmul (:shadow-map-matrix (shadow-matrices projection transform2 light-direction 500)) (matrix [0 1500 0 1]))
         => (roughly-matrix (matrix [0.5 0.5 1 1]) 1e-6)
...
(facts "Cascade of shadow matrices"
       (let [near            10
             ...
             light-direction (matrix [0 1 0])]
         (mmul (:shadow-ndc-matrix (nth (shadow-matrix-cascade projection transform light-direction 0 0 near far 2) 0))
               (matrix [0 0 -10 1]))
         => (roughly-matrix (matrix [-1 0 0.5 1]) 1e-6)
         (mmul (:shadow-ndc-matrix (nth (shadow-matrix-cascade projection transform light-direction 0 0 near far 2) 0))
               (matrix [0 0 -25 1]))
         => (roughly-matrix (matrix [1 0 0.5 1]) 1e-6)
         (mmul (:shadow-ndc-matrix (nth (shadow-matrix-cascade projection transform light-direction 0 1 near far 2) 0))
               (matrix [0 0 -20 1]))
         => (roughly-matrix (matrix [1 0 0.5 1]) 1e-6)
         ...
         => (roughly 47.5 1e-6)
         (mmul (:shadow-ndc-matrix (nth (shadow-matrix-cascade projection transform light-direction 0 0 near far 2) 1))
               (matrix [0 0 -25 1]))
         => (roughly-matrix (matrix [-1 0 0.5 1]) 1e-6)))
