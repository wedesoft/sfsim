...
(facts "Choose NDC and texture coordinate matrices for shadow mapping"
       (let [projection      (projection-matrix 640 480 5.0 1000.0 (* 0.5 PI))
         ...
         (fm/mulv (:shadow-ndc-matrix (shadow-matrices projection transform2 light-direction 0)) (fv/vec4 0 1000 0 1))
         => (roughly-vector (fv/vec4 0 0 1 1) 1e-6)
         (fm/mulv (:shadow-ndc-matrix (shadow-matrices projection transform2 light-direction 0)) (fv/vec4 0 5 0 1))
         => (roughly-vector (fv/vec4 0 0 0 1) 1e-6)
         (fm/mulv (:shadow-map-matrix (shadow-matrices projection transform1 light-direction 0)) (fv/vec4 0 -750 -1000 1))
         => (roughly-vector (fv/vec4 1 0.5 0 1) 1e-6)
         (fm/mulv (:shadow-map-matrix (shadow-matrices projection transform1 light-direction 0)) (fv/vec4 0 750 -1000 1))
         => (roughly-vector (fv/vec4 1 0.5 1 1) 1e-6)
         (fm/mulv (:shadow-map-matrix (shadow-matrices projection transform2 light-direction 0)) (fv/vec4 0 1000 0 1))
         => (roughly-vector (fv/vec4 0.5 0.5 1 1) 1e-6)
         (fm/mulv (:shadow-map-matrix (shadow-matrices projection transform2 light-direction 500)) (fv/vec4 0 1500 0 1))
         => (roughly-vector (fv/vec4 0.5 0.5 1 1) 1e-6)
