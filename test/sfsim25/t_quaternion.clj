...
(defn roughly-vector [y error] (fn [x] (<= (l/norm (sub y x)) error)))

(facts "Rotate a vector using a rotation quaternion"
  (rotate-vector (rotation 0 (matrix [1 0 0])) (matrix [2 4 8]))        => (roughly-vector (matrix [2  4 8]) 1e-6)
  (rotate-vector (rotation (/ pi 2) (matrix [1 0 0])) (matrix [2 4 8])) => (roughly-vector (matrix [2 -8 4]) 1e-6))
