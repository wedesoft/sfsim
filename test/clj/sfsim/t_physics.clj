...
(facts "Get orbital parameters"
       (let [planet {:sfsim.planet/mass 5.9722e+24 :sfsim.planet/radius 6378000.0 :sfsim.planet/max-height 8000.0}]
         ...
         (:sfsim.physics/velocity (orbital-parameters planet @state)) => (roughly 9000.0 1e-3)
         ...
         (:sfsim.physics/semi-major-axis (orbital-parameters planet @state)) => (roughly 10290123.277 1e-3)
         (semi-minor-axis planet @state) => (roughly 9627788.820 1e-3)
         (:sfsim.physics/semi-minor-axis (orbital-parameters planet @state)) => (roughly 9627788.820 1e-3)
         ...
         (let [a         (semi-major-axis planet @state)
           ...
           (eccentricity planet @state) => (roughly 0.352972 1e-6)
           (:sfsim.physics/eccentricity (orbital-parameters planet @state)) => (roughly 0.352972 1e-6)
           (true-anomaly planet @state) => (roughly (/ PI 2) 1e-6)
           (:sfsim.physics/true-anomaly (orbital-parameters planet @state)) => (roughly (/ PI 2) 1e-6)
           ...
           (:sfsim.physics/time-since-periapsis (orbital-parameters planet @state)) => (roughly 1454.602 1e-3)
           (time-since-apoapsis planet @state) => (roughly (- 1454.602 (/ 10388.210 2.0)) 1e-3)
           (:sfsim.physics/time-since-apoapsis (orbital-parameters planet @state)) => (roughly (- 1454.602 (/ 10388.210 2.0)) 1e-3)
         ...
         (argument-of-periapsis planet @state) => (roughly 0.0 1e-6)
         (:sfsim.physics/inclination (orbital-parameters planet @state)) => (roughly (/ PI 2) 1e-6)
         (swap! state set-speed :sfsim.physics/orbit (vec3 0 -9000 0) (vec3 0 0 0))
         ...
         (:sfsim.physics/longitude-ascending-node (orbital-parameters planet @state)) => (roughly (to-radians 90.0) 1e-6)
         (swap! state set-pose :sfsim.physics/orbit (vec3 0 -6658000 0) (q/->Quaternion 1 0 0 0))
         (longitude-ascending-node @state) => (roughly (to-radians 270.0) 1e-6)
         (swap! state set-pose :sfsim.physics/orbit (vec3 0 0 6658000) (q/->Quaternion 1 0 0 0))
         (swap! state set-speed :sfsim.physics/orbit (vec3 -9000 0 0) (vec3 0 0 0))
         (argument-of-periapsis planet @state) => (roughly (/ PI 2) 1e-6)
         (:sfsim.physics/argument-of-periapsis (orbital-parameters planet @state)) => (roughly (/ PI 2) 1e-6)
         (swap! state set-pose :sfsim.physics/orbit (vec3 0 0 -6658000) (q/->Quaternion 1 0 0 0))
         (swap! state set-speed :sfsim.physics/orbit (vec3 9000 0 0) (vec3 0 0 0))
