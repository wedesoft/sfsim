...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  if (box.y > 0.0) {
    ...
    for (int i = 0; i <= SAMPLES; i++)
    {
      ...
      vec2 envelope = envelope(pressure, START - p.x - mix(ENGINE_SIZE, 0.0, transition));
      float engine_pos = clamp((START - p.x) / ENGINE_SIZE, 0.0, 1.0);
      ...
      float fade = clamp((p.x - END) / (START - END), 0.0, 1.0);
      float shapeMix = mix(sdf_circle(p.zy, vec2(0, 0), radius), sdf_rectangle(p.zy, -envelope, +envelope), fade);
      ...
      if (sdf < 0.0) {
        ...
        float diamond = mix(diamond_strength, diamond(pressure, vec2(START - p.x - mix(ENGINE_SIZE, 0.0, transition), max(0.0, sdf + dy))), mix(engine_pos, 1.0, transition));
