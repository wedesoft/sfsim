...
(def transmittance-shape [transmittance-height-size transmittance-elevation-size])
...
(def table (mapv (fn [row] (map matrix (partition 3 row))) (partition (* 3 surface-sun-elevation-size) data)))
(def surface-shape [surface-height-size surface-sun-elevation-size])
(def surface-space-earth (surface-radiance-space earth surface-shape))
(def forward (:sfsim25.interpolate/forward surface-space-earth))
(def backward (:sfsim25.interpolate/backward surface-space-earth))
(def E (interpolation-table table surface-space-earth))
