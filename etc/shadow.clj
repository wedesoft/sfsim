...
(def pixels (BufferUtils/createIntBuffer (* width height)))
(def pbuffer (Pbuffer. width height (PixelFormat. 24 8 24 0 0) nil nil))
(.makeCurrent pbuffer)
...
(GL42/glTexStorage2D GL11/GL_TEXTURE_2D 1 GL30/GL_DEPTH_COMPONENT32F size size)
...
(show-floats {:width size :height size :data (float-array (map #(* 255 %) (:data img)))})
