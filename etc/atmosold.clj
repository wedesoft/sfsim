...
(def profile-size 12)
(def worley-size 64)
(def octaves [0.5 0.25 0.125 0.0625 0.0625])
(def noise-size 64)
(def depth 120000.0)
(def fov 45.0)
(def height-size 32)
...
(def cloud-multiplier (atom 0.05))
(def threshold (atom -0.3))
(def mix (atom 0.8))
...
(doseq [i (range num-steps)]
       ...
       (uniform-sampler program-atmosphere (str "opacity" i) (+ (* 2 i) 7)))
    ...
    (fn [{:keys [shadow-ndc-matrix depth]}]
        (let [opacity-offsets (make-empty-float-texture-2d :linear :clamp shadow-size shadow-size)
          ...
          (framebuffer-render shadow-size shadow-size :cullback nil [opacity-offsets opacity-layers]
                              ...
                              (uniform-int program-shadow "cloud_size" worley-size)
