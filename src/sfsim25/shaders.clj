...
(def percentage-closer-filtering
  ...
  (template/fn [point-type method-name base-function-name parameters]
               (slurp (io/resource "shaders/core/percentage-closer-filtering.glsl"))))

(def make-2d-index-from-4d
  "Convert 4D index to 2D indices for part-manual interpolation"
  (slurp (io/resource "shaders/core/make-2d-index-from-4d.glsl")))

(def interpolate-2d
  "Perform 2D color interpolation"
  (slurp (io/resource "shaders/core/interpolate-2d.glsl")))

(def interpolate-3d
  "Perform 3D float interpolation"
  (slurp (io/resource "shaders/core/interpolate-3d.glsl")))

(def interpolate-cubemap
  "Perform interpolation on cubemap avoiding seams"
  (template/fn [result-type method-name selector] (slurp (io/resource "shaders/core/interpolate-cubemap.glsl"))))
