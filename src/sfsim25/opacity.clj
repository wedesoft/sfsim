...
(defn make-opacity-renderer
  ...
  [& {:keys [num-opacity-layers shadow-size radius cloud-data]}]
  (let [program  (make-program :vertex [opacity-vertex]
                               :fragment [(opacity-fragment num-opacity-layers
...
(defn render-opacity-cascade
  ...
  [{:keys [program vao shadow-size num-opacity-layers cloud-data]}
  ...
  (opacity-cascade shadow-size num-opacity-layers matrix-cas (/ (:detail-scale cloud-data) worley-size) program
