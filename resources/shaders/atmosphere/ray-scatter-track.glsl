...
vec3 ray_scatter_track(vec3 light_direction, vec3 p, vec3 q)
{
  ...
  if (dist > 0) {
    ...
    vec3 ray_scatter_p =
      interpolate_4d(ray_scatter, height_size, elevation_size, light_elevation_size, heading_size, ray_scatter_index_p).rgb +
      interpolate_4d(mie_strength, height_size, elevation_size, light_elevation_size, heading_size, ray_scatter_index_p).rgb *
      phase(0.76, mu);
    vec4 ray_scatter_index_q = ray_scatter_forward(q, direction, light_direction, above_horizon);
    vec3 ray_scatter_q =
      interpolate_4d(ray_scatter, height_size, elevation_size, light_elevation_size, heading_size, ray_scatter_index_q).rgb +
      interpolate_4d(mie_strength, height_size, elevation_size, light_elevation_size, heading_size, ray_scatter_index_q).rgb *
      phase(0.76, mu);
