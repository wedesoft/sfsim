...
vec3 transmittance_track(sampler2D transmittance, float radius, float max_height, int size, float power, vec3 p, vec3 q)
{
  ...
  if (dist > 0) {
    ...
    vec2 uvp = transmittance_forward(p, direction, radius, max_height, size, power, above_horizon);
    vec2 uvq = transmittance_forward(q, direction, radius, max_height, size, power, above_horizon);
