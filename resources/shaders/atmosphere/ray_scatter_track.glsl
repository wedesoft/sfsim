                       ...
                       vec3 light_direction, vec3 p, vec3 q)
{
  ...
  if (dist > 0) {
    ...
    vec4 ray_scatter_index_p = ray_scatter_forward(p, direction, light_direction, radius, max_height, size, power, false);
    vec3 ray_scatter_p = interpolate_4d(ray_scatter, size, ray_scatter_index_p).rgb;
    vec4 ray_scatter_index_q = ray_scatter_forward(q, direction, light_direction, radius, max_height, size, power, false);
