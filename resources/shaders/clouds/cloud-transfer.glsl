...
vec4 cloud_transfer(vec3 start, vec3 point, float scatter_amount, float stepsize, vec4 cloud_scatter, float density)
{
  ...
  vec4 cloud_color = vec4(illumination * scatter_amount * powder(density * stepsize), 1.0 - cloud_transmittance);
  cloud_scatter.rgb += cloud_scatter.a * cloud_color.a * attenuate(light_direction, start, point, cloud_color.rgb);
