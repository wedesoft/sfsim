...
{
  int count = number_of_samples(atmosphere.x, atmosphere.x + atmosphere.y, cloud_step);
  float stepsize = step_size(atmosphere.x, atmosphere.x + atmosphere.y, count);
  ...
  for (int i=0; i<count; i++) {
    if (cloud_scatter.a <= opacity_cutoff)
      break;
    float l = sample_point(atmosphere.x, i + noise, stepsize);
