...
float elevation_to_index(int size, float elevation, float horizon_angle, float power, bool above_horizon) {
  ...
  if (above_horizon) // sky
    result = (1 - pow(1 - min(elevation, horizon) / horizon, 1 / power)) * (sky_size - 1);
  else // ground
    result = sky_size + pow((max(elevation, horizon) - horizon) / (0.5 * M_PI - horizon_angle), 1 / power) * (ground_size - 1);
